<template>
  <el-table
    ...
    <template v-for="(column,index) in columns">
      <template v-if="column.children && column.children.length>0">
        <el-table-column :key="column" :prop="column.prop" :label="column.label">
          <template v-for="childColumn in column.children">
            <el-table-column :key="childColumn" :prop="childColumn.prop" :label="childColumn.label"
                             :min-width="childColumn.width>width?childColumn.width:width">
      ...
      <template v-else>
        <el-table-column :key="column" align="center" :prop="column.prop" :label="column.label"
                         :min-width="column.width>width?column.width:width">
          <template slot="header" slot-scope="slot">
